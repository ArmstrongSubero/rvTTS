...
void word_vocab_assign_frames(
    ...
    char *words[512];
    uint16_t word_ids_list[512];
    uint32_t n_words = 0;

    char *tok = strtok(buf, " \t\n\r");
    while (tok && n_words < 512) {
        words[n_words] = tok;
        word_ids_list[n_words] = word_vocab_lookup(tok);
        n_words++;
        tok = strtok(NULL, " \t\n\r");
    }
    ...
    for (uint32_t t = 0; t < frame_count; t++) {
        if (frame_phones[t] == PH_SIL || frame_phones[t] == PH_BRK) {
            out_word_ids[t] = 0;
            if (in_word) {
                word_idx++;
                in_word = 0;
            ...
            if (word_idx < n_words) {
                out_word_ids[t] = word_ids_list[word_idx];
            } else {
                out_word_ids[t] = 0;
            }
        }
